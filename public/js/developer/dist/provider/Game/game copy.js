...
class Game {
  ...
  async init() {
    ...
    const thirdPersonCamera = thirdPersonCameraInstance.camera;
    ...
    const controls3 = this.orbitControls({camera:thirdPersonCamera, render: rendererR});
    ...
    const material_character = new THREE.MeshBasicMaterial({ color: 0x00ff00 , wireframe: true});
    const character = new THREE.Mesh(geometry_character, material_character);
    // Position the character
    character.position.set(0, 0.8, 10);


    loaderGL.load('../../obj/idle.glb', function (gltf) {
      let model = gltf.scene;
      var clips = gltf.animations;
