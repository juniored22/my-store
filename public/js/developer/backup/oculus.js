...
class Oculus{
    ...
    async processImgCanvas(){
        ...
        const processedImg = this.preprocessImage(tfImage);
    ...
    preprocessImage(imageTensor) {
        return tf.tidy(() => {
            ...
            // console.log({'antes': processedImg});

        
            // Ajuste de brilho e contraste
            processedImg = this.adjustBrightnessContrast(processedImg, -50, 1);

            // Conversão para escala de cinza
            // processedImg = this.convertToGrayscale(processedImg);


            // Aplicar desfoque gaussiano TODO:codigo quebrado
            // processedImg = this.applyGaussianBlur(processedImg);

            // processedImg = this.codeCuston(processedImg);

            // processedImg = this.detectEdges(processedImg);

            
           
            // processedImg = this.dilation(processedImg);

            // processedImg = this.rgbToHsvTensor(processedImg);

            // processedImg = this.filterAndReplaceColors(processedImg, [35, 50, 50], [85, 100, 100]);

            // Normalização
            // processedImg = this.normalize(processedImg);

            // Redimensionar para a entrada do modelo
            // processedImg = tf.image.resizeBilinear(processedImg, [128, 128]);
    ...
    async createModuleCustom() {
        ...
        console.log({nameModules: this.nameModules});
        ...
        // iinput x
        // Convert tensors to float32 channels 3 RGB
        // const flattenedTensors = this.tensors.map(tensor => tensor.squeeze().div(255.0))
        // const xs = tf.stack(flattenedTensors);

        // Convert tensors to float32 channels 1 gray to RGB
        const tensorsWithThreeChannels = this.tensors.map(tensor => tf.image.grayscaleToRGB(tensor));
        const xs = tf.concat(tensorsWithThreeChannels).reshape([this.tensors.length, 224, 224, 3]).toFloat();
        ...
        console.log({ xs, ys });
        console.log({ xsshape: xs.shape, ysshape: ys.shape });
    ...
    async predictModel() {
        ...
        console.log('%c[Oculus][predictModel]', 'background: #36447b;color:#fff');
        ...
        const predictModelRecursive = async () => {
            ...
            const tensors = [];
            try {
                const tensor = tf.browser.fromPixels(this.optionsVideo.video.element, 1);
                const box = [this.cropBox.y, this.cropBox.x, this.cropBox.y + this.cropBox.height, this.cropBox.x + this.cropBox.width];
                const cropTensor = tf.image.cropAndResize(tensor.expandDims(0), [box], [0], [224, 224]).toFloat(); //.div(tf.scalar(255.0))
                tensors.push(cropTensor.div(tf.scalar(255.0)));
    ...
    updateCropBox() {
        ...
        // Draw crop box
        this.contextTrainModel.strokeStyle = 'blue';
    ...
    async getCropBoxToData({classid}) {
        ...
        this.cropTensors.push(cropTensor);
        ...
        this.nameModules.save['lastTensor'] = tensor;
        this.nameModules.save['lastCropTensor'] = cropTensor;
    ...
    async buildMenu() {
        ...
        return new Promise( async (resolve) => {
                ...
                const countdownInterval = setInterval(() => {
                    ...
                    if (timeout === 0) {
                        ...
                        const captureInterval = setInterval( async () => {
                            ...
                            /* 
                            debugger
                            // const cropSize = 100; // Specify the size of the crop region
                            // const cropBoxPixels = {
                            //     x: this.cropBox.x * tensor.shape[1],
                            //     y: this.cropBox.y * tensor.shape[0],
                            //     width: this.cropBox.width * tensor.shape[1],
                            //     height: this.cropBox.height * tensor.shape[0],
                            // };
                            // const box = [0.5 - cropSize / 2 / tensor.shape[0], 0.5 - cropSize / 2 / tensor.shape[1], 0.5 + cropSize / 2 / tensor.shape[0], 0.5 + cropSize / 2 / tensor.shape[1]];

                            // const rne = tf.image.resizeBilinear(tensor, [224, 224]).div(255.0);
                            const rne = tf.image.resizeBilinear(tensor, [224, 224]).toFloat().div(tf.scalar(255.0));

                            //this.tensors.push(rne.expandDims(0));
                            this.tensors.push(cropTensor.div(tf.scalar(255.0)));
                            this.labels.push(this.classMap[classid]);

                            console.log('%c[Oculus][buildMenu]', 'background: green; color: #fff', tensor);

                            // Render the cropped image on canvas
                            const canvas = document.createElement('canvas');
                            canvas.width = 224;
                            canvas.height = 224;

                            const displayTensor = cropTensor.mul(tf.scalar(255)).clipByValue(0, 255).toInt();
                            ...
                            */
                            ...
                            captureTimeout++;
                            if (captureTimeout === 10) {
                                clearInterval(captureInterval);
                                
                                // this.stackedTensor = tf.stack(this.tensors);
                                
                                inputClassId.value = '';
                                // console.log('Captured tensors:', this.tensors);
                                // console.log('stackedTensor:', this.stackedTensor);
            ...
            div.addEventListener('mousedown', (e) => {
                ...
                console.log('mousedown', e, {offsetX, offsetY, div}, div.getBoundingClientRect());
